...
const TEXT_FIELDS = [
  ...
  { key: 'about_intro',       label: 'Introduction',        rows: 4 },
  { key: 'about_mission',     label: 'Mission Statement',   rows: 4 },
  { key: 'about_card1_title', label: 'Highlight 1 — Title' },
  { key: 'about_card1_desc',  label: 'Highlight 1 — Description', rows: 2 },
  { key: 'about_card2_title', label: 'Highlight 2 — Title' },
  { key: 'about_card2_desc',  label: 'Highlight 2 — Description', rows: 2 },
  { key: 'about_card3_title', label: 'Highlight 3 — Title' },
  { key: 'about_card3_desc',  label: 'Highlight 3 — Description', rows: 2 },
]
...
export default function AdminAboutEditor() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const f = {}
      for (const { key } of TEXT_FIELDS) f[key] = settings[key] ?? ''
      setFields(f)
      fieldsRef.current = f
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="rounded-xl border border-stone-200 bg-white p-6 shadow-sm space-y-5">
          <h2 className="font-bold text-stone-900 text-sm uppercase tracking-wide">Content</h2>

          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            {TEXT_FIELDS.map(({ key, label, rows }) => (
              <div key={key} className={rows && rows >= 4 ? 'md:col-span-2' : ''}>
        ...
        <div className="rounded-xl border border-stone-200 bg-white p-6 shadow-sm space-y-4">
          <h2 className="font-bold text-stone-900 text-sm uppercase tracking-wide">Photos</h2>
          <p className="text-xs text-stone-500">The first photo uploaded becomes the page hero banner. Additional photos appear as a gallery below the content.</p>

          {images.length > 0 && (
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {images.map((img, idx) => (
                <div key={img.id} className="group relative rounded-xl overflow-hidden border border-stone-200 aspect-square bg-stone-100">
                  <img src={img.url} alt={img.alt_text || ''} className="w-full h-full object-cover" />
                  {idx === 0 && (
                    <span className="absolute top-1 left-1 bg-black/60 text-white text-[9px] font-bold uppercase px-1.5 py-0.5 rounded">Hero</span>
                  )}
                  <button
            ...
            {uploading
              ...
              : (
                <>
                  ...
                  <p className="text-sm text-stone-500">Drop a photo here or click to upload</p>
                  <p className="text-xs text-stone-400 mt-1">JPG, PNG, WebP — max 10 MB</p>
                </>
              )
            }
          </div>
